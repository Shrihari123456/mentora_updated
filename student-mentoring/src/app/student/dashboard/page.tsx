...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        ...
        console.error('Failed to parse student data from localStorage');
      }
    }
    setLoading(false);
  }, []);
  ...
  const studentSrNo  = studentData?.srNo || '';
  ...
  const handleSearch = () => {
    if (searchLocation.trim()) fetchEvents(searchLocation.trim());
  };
  ...
  const handlePDFUpload = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/api/chatbot/upload', { method: 'POST', body: formData });
      if (!response.ok) throw new Error(`Server responded with ${response.status}`);
      const data = await response.json();
      if (data.success) {
        setUploadedPDF({ id: `pdf_${Date.now()}`, name: file.name, size: file.size, text: data.data.text, uploadedAt: new Date() });
        setChatMessages(prev => [...prev, {
          id: `pdf_upload_${Date.now()}`, message: '',
          response: `📄 **PDF Uploaded Successfully!**\n\nFile: ${file.name}\nSize: ${(file.size / 1024).toFixed(2)} KB\n\nYou can now ask questions about its content!`,
          timestamp: new Date(), type: 'bot'
        }]);
      }
    } catch (error: any) {
  ...
  const handleSendMessage = async () => {
    ...
    setChatMessages(prev => [...prev, { id: tempId, message: userMessage, response: '', timestamp: new Date(), type: 'student' }]);
    setIsLoading(true);
    try {
      const requestBody: any = { message: userMessage };
      if (uploadedPDF?.text) requestBody.pdfContext = { text: uploadedPDF.text, filename: uploadedPDF.name };
      ...
      setChatMessages(prev => [
        ...
        { id: `bot_${Date.now()}`, message: userMessage, response: data.success ? data.data.response : "⚠️ I'm having trouble. Please try again.", timestamp: new Date(), type: 'bot' }
      ]);
    } catch {
      setChatMessages(prev => [
        ...prev.filter(msg => msg.id !== tempId),
        { id: `error_${Date.now()}`, message: userMessage, response: "🔌 Network error. Please try again.", timestamp: new Date(), type: 'bot' }
      ]);
    } finally {
      setIsLoading(false);
    }
  ...
  const clearChat = () => {
    if (confirm('Start a new conversation?')) { setChatMessages([]); setUploadedPDF(null); initializeChatSession(); }
  };
  ...
  const handleStudentSearch = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/report/search', {
        ...
        body: JSON.stringify({ query: searchQuery, studentId: studentData?._id || '' })
      });
      const data = await response.json();
      if (data.success) { setSearchResults(data.data.students); setSearchExplanation(data.data.explanation); }
      else alert('Search failed: ' + data.message);
    } catch { alert('Error performing search'); }
    finally { setIsSearching(false); }
  };

  const handleSearchKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleStudentSearch(); }
  };
  ...
  useEffect(() => { fetchEvents('Bangalore'); }, []);
  ...
  const dashboardCards = [
    ...
    {
      title: "Mail my Mentor",
      description: "Mailing and receiving mails from mentor",
      icon: <Mail size={24} />,
      color: "bg-gradient-to-r from-indigo-100 to-blue-50 text-indigo-600",
    ...
    >
      <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-gray-600">
            ...
            {studentSrNo && (
              <> · SR No: <span className="font-semibold text-indigo-600">{studentSrNo}</span></>
            )}
            {studentUSN && studentUSN !== studentSrNo && (
              <> · USN: <span className="font-semibold text-indigo-600">{studentUSN}</span></>
            )}
          </p>
          <div className="inline-flex items-center mt-2 px-4 py-1.5 bg-indigo-100 text-indigo-700 rounded-full text-sm font-medium">
            ...
            Active Student • {studentData?.section ? `Section ${studentData.section}` : 'Semester 1'}
          </div>
        </div>
        ...
        <div className="mb-8 flex flex-wrap gap-4 justify-center">
          <button
            onClick={() => router.push('/student/studentEmail')}
                  ...
                  {events.filter(e => e.type === 'hackathon').slice(0, 3).map(event => (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between text-sm">
                        ...
                        {event.link && <span className="text-purple-600 font-medium group-hover:underline">Register →</span>}
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-gradient-to-br from-purple-900 to-indigo-900 rounded-2xl shadow-lg p-6 text-white">
              ...
              <div className="space-y-3 mb-6">
                ...
                {cseQuickPrompts.slice(0, 4).map((prompt, idx) => (
                  <button key={idx} onClick={() => handleQuickPrompt(prompt.text)}
                    className="w-full text-left p-3 bg-white/10 hover:bg-white/20 rounded-xl transition-all duration-300 group">
                    <div className="flex items-center gap-3">
                      <div className="p-1.5 bg-white/20 rounded-lg group-hover:bg-white/30">{prompt.icon}</div>
                      <div className="flex-1">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
              <h2 className="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                <Award className="text-yellow-600" size={24} /> Recent Achievements
              </h2>
      ...
      {isStudentSearchOpen && (
        <StudentSearchModal
          ...
          onClose={() => { setIsStudentSearchOpen(false); setSearchQuery(''); setSearchResults([]); setSearchExplanation(''); }}
...
function EmailModal({ isOpen, onClose, studentId, mentorId, studentUSN, studentEmail }: {
  isOpen: boolean; onClose: () => void; studentId: string; mentorId: string; studentUSN: string; studentEmail?: string;
}) {
  ...
  const handleSendEmail = async () => {
    if (!fromEmail.trim() || !subject.trim() || !message.trim()) { setStatus({ type: 'error', message: 'Please fill in all fields' }); return; }
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(fromEmail)) { setStatus({ type: 'error', message: 'Please enter a valid email address' }); return; }
    setLoading(true);
    setStatus({ type: null, message: '' });
    try {
      const response = await fetch('http://localhost:8000/api/email/mentee-to-mentor', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ studentId, mentorId, subject, message, studentEmail: fromEmail })
      });
      const data = await response.json();
      if (response.ok) {
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          ...
          <button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-lg transition"><X size={24} className="text-gray-500" /></button>
        </div>
        <div className="p-6 space-y-6">
          {status.type && (
            <div className={`p-4 rounded-xl ${status.type === 'success' ? 'bg-green-50 text-green-800 border border-green-200' : 'bg-red-50 text-red-800 border border-red-200'}`}>
              <div className="flex items-center gap-2"><AlertCircle size={20} />{status.message}</div>
            </div>
          )}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">USN / SR No</label>
              <input type="text" value={studentUSN} disabled className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl bg-gray-50 text-gray-600" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Your Email *</label>
              <input type="email" value={fromEmail} onChange={e => setFromEmail(e.target.value)} placeholder="your.email@example.com"
          ...
          <div>
            ...
            <input type="text" value={subject} onChange={e => setSubject(e.target.value)} placeholder="What is this regarding?"
              className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Message *</label>
            <textarea value={message} onChange={e => setMessage(e.target.value)} placeholder="Write your detailed message here..." rows={6}
            ...
            <div className="flex justify-between mt-2">
              <p className="text-sm text-gray-500">Your mentor will reply to your email address</p>
              <p className="text-sm text-gray-500">{message.length} characters</p>
            </div>
          </div>
        </div>
        <div className="flex justify-end gap-3 p-6 border-t border-gray-200 bg-gray-50 rounded-b-2xl">
          <button onClick={onClose} disabled={loading} className="px-6 py-3 border-2 border-gray-300 text-gray-700 rounded-xl hover:bg-gray-100 transition font-medium">Cancel</button>
          <button onClick={handleSendEmail} disabled={loading || !fromEmail.trim() || !subject.trim() || !message.trim()}
            ...
            {loading ? <><div className="animate-spin h-5 w-5 border-2 border-white border-t-transparent rounded-full" />Sending...</> : <><Send size={18} />Send Email</>}
          </button>
        </div>
      </div>
    </div>
...
// ─── Chat Modal ───────────────────────────────────────────────────────────────
function ChatModal({
  onClose, messages, chatInput, setChatInput, isLoading, onSendMessage, onQuickPrompt,
  onClearChat, onKeyDown, chatContainerRef, inputRef, quickPrompts, studentName,
  uploadedPDF, isUploading, uploadProgress, onPDFUpload, fileInputKey, setFileInputKey
}: {
  onClose: () => void; messages: ChatMessage[]; chatInput: string; setChatInput: (i: string) => void;
  isLoading: boolean; onSendMessage: () => void; onQuickPrompt: (p: string) => void; onClearChat: () => void;
  onKeyDown: (e: React.KeyboardEvent) => void; chatContainerRef: React.RefObject<HTMLDivElement>;
  inputRef: React.RefObject<HTMLTextAreaElement>; quickPrompts: CSEQuickPrompt[]; studentName: string;
  uploadedPDF: UploadedPDF | null; isUploading: boolean; uploadProgress: number;
  onPDFUpload: (f: File) => Promise<void>; fileInputKey: number; setFileInputKey: (k: number) => void;
}) {
  ...
  const handleUploadClick = () => {
    ...
    input.onchange = (e) => {
      ...
      if (file) { setSelectedFile(file); onPDFUpload(file); }
    };
    input.click();
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-0">
      <div className={`bg-gradient-to-br from-gray-900 to-black shadow-2xl ${isFullScreen ? 'w-screen h-screen rounded-none' : 'max-w-6xl w-full h-[90vh] lg:h-[85vh] rounded-2xl'} transition-all duration-300 flex flex-col`}>
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-white/10 bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900">
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              <div className="relative p-2 bg-gray-900 rounded-xl border border-white/10"><Bot className="text-white" size={24} /></div>
            </div>
            <h2 className="text-xl font-bold text-white flex items-center gap-2">
              CodeMentor AI
              <span className="px-2 py-0.5 bg-gradient-to-r from-purple-600 to-cyan-500 text-xs rounded-full">Beta</span>
            </h2>
          </div>
          <div className="flex items-center gap-1">
            <button onClick={() => setIsFullScreen(!isFullScreen)} className="p-2 hover:bg-white/10 rounded-lg transition text-gray-400 hover:text-white">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5v-4m0 4h-4m4 0l-5-5" />
              </svg>
            </button>
            <button onClick={onClearChat} className="p-2 hover:bg-white/10 rounded-lg transition text-gray-400 hover:text-red-400"><Trash2 size={20} /></button>
            <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-lg transition text-gray-400 hover:text-white"><X size={24} /></button>
          </div>
        </div>
          ...
          {(uploadedPDF || selectedFile) && (
            <div className="px-4 py-2 border-b border-white/10 bg-gray-800/50">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="p-1.5 bg-gradient-to-r from-cyan-600 to-blue-600 rounded-lg"><FileText className="text-white" size={16} /></div>
                  <div>
                    <p className="text-sm font-medium text-white">{uploadedPDF?.name || selectedFile?.name}</p>
                    <p className="text-xs text-gray-400">{uploadedPDF ? `${(uploadedPDF.size / 1024 / 1024).toFixed(2)} MB • Ready` : 'Processing...'}</p>
                  </div>
                </div>
                <button onClick={() => setSelectedFile(null)} className="p-1 hover:bg-white/10 rounded text-gray-400 hover:text-red-400"><X size={16} /></button>
              </div>
            </div>
          )}

          <div ref={chatContainerRef} className="flex-1 overflow-y-auto p-4 md:p-6 bg-gradient-to-b from-gray-900 to-black">
            {messages.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center py-8">
                ...
                <h1 className="text-3xl font-bold text-white mb-2 text-center">Hello, {studentName}! 👋</h1>
                <p className="text-gray-400 text-center max-w-lg mb-8">I'm your AI programming assistant. Upload a PDF or start chatting!</p>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 max-w-4xl w-full mb-8">
                  {quickPrompts.slice(0, 6).map((prompt, idx) => (
                    <button key={idx} onClick={() => onQuickPrompt(prompt.text)}
                      ...
                      <div className="flex items-center gap-3 mb-2">
                        <div className="p-2 bg-gradient-to-r from-purple-600/20 to-cyan-600/20 rounded-lg">{prompt.icon}</div>
                ...
                <div
                  className={`max-w-md w-full p-8 border-2 border-dashed rounded-2xl cursor-pointer transition-all ${dragOver ? 'border-cyan-500 bg-cyan-500/10' : 'border-white/20 hover:border-cyan-500/50'}`}
                  onDragOver={e => { e.preventDefault(); setDragOver(true); }}
                  onDragLeave={() => setDragOver(false)}
                  onDrop={e => { e.preventDefault(); setDragOver(false); const f = e.dataTransfer.files[0]; if (f) { setSelectedFile(f); onPDFUpload(f); } }}
                ...
                >
                  <div className="text-center">
                    ...
                    <p className="text-gray-400 text-sm">Lecture notes, research papers, assignments</p>
                    <p className="text-xs text-gray-500 mt-2">Drag & drop or click • Max 10MB</p>
                  </div>
                </div>
              </div>
            )}
                ...
                <div key={msg.id} className={`flex ${msg.type === 'student' ? 'justify-end' : 'justify-start'}`}>
                  {msg.type === 'student' ? (
                    <div className="max-w-[85%]">
                      <div className="bg-gradient-to-r from-purple-600 to-cyan-600 text-white rounded-2xl rounded-br-none p-5">
                    ...
                    <div className="max-w-[85%]">
                      <div className="flex items-start gap-3 mb-2">
                        <div className="p-2 bg-gradient-to-r from-purple-600 to-cyan-600 rounded-xl flex-shrink-0"><Bot className="text-white" size={16} /></div>
                        <div><p className="text-sm font-medium text-gray-300">CodeMentor AI</p><p className="text-xs text-gray-500">AI Assistant</p></div>
                      </div>
                      <div className="bg-gray-800/50 border border-white/10 rounded-2xl rounded-tl-none p-5">
                        <div className="prose prose-invert max-w-none text-gray-200" dangerouslySetInnerHTML={{ __html: formatBotResponse(msg.response) }} />
                      </div>
                      <span className="text-xs text-gray-500">{msg.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
                  )}
              ...
              {isLoading && (
                <div className="flex justify-start">
                  <div className="max-w-[85%]">
                    <div className="flex items-start gap-3 mb-2">
                      <div className="p-2 bg-gradient-to-r from-purple-600 to-cyan-600 rounded-xl flex-shrink-0"><Bot className="text-white" size={16} /></div>
                      <div><p className="text-sm font-medium text-gray-300">CodeMentor AI</p><p className="text-xs text-gray-500">Thinking...</p></div>
                    </div>
                    <div className="bg-gray-800/50 border border-white/10 rounded-2xl rounded-tl-none p-5">
                      <div className="flex items-center gap-3">
                        <div className="flex space-x-1">
                          {[0, 0.2, 0.4].map((delay, i) => (
                            <div key={i} className="w-2 h-2 bg-cyan-500 rounded-full animate-pulse" style={{ animationDelay: `${delay}s` }}></div>
                          ))}
                        </div>
            ...
            <div className="max-w-4xl mx-auto">
              ...
              <div className="relative">
                <textarea ref={inputRef} value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={onKeyDown}
                  placeholder={uploadedPDF ? `Ask questions about "${uploadedPDF.name}"...` : "Message CodeMentor AI..."}
                  className="w-full px-4 py-3 pr-12 bg-gray-800/50 border border-white/10 rounded-xl focus:outline-none focus:border-cyan-500/50 text-white placeholder-gray-500 resize-none text-base"
                  rows={2} disabled={isLoading} />
                <div className="absolute right-2 bottom-2">
                  <button onClick={onSendMessage} disabled={isLoading || !chatInput.trim()}
                    className="p-2 bg-gradient-to-r from-purple-600 to-cyan-600 text-white rounded-lg hover:shadow-lg transition disabled:opacity-50">
                    <Send size={18} />
                  </button>
                </div>
              </div>
              <div className="flex justify-between mt-2 px-1">
                ...
                <p className="text-xs text-gray-500">{chatInput.length}/2000</p>
              </div>
            </div>
          </div>
        </div>
...
function StudentSearchModal({
  isOpen, onClose, searchQuery, setSearchQuery, searchResults, setSearchResults,
  isSearching, setIsSearching, searchExplanation, setSearchExplanation,
  studentData, exampleQueries, handleStudentSearch, handleSearchKeyDown
}: {
  isOpen: boolean; onClose: () => void; searchQuery: string; setSearchQuery: (q: string) => void;
  searchResults: StudentSearchResult[]; setSearchResults: (r: StudentSearchResult[]) => void;
  isSearching: boolean; setIsSearching: (s: boolean) => void;
  searchExplanation: string; setSearchExplanation: (e: string) => void;
  studentData: any; exampleQueries: { query: string; icon: JSX.Element }[];
  handleStudentSearch: () => Promise<void>; handleSearchKeyDown: (e: React.KeyboardEvent) => void;
}) {
  if (!isOpen) return null;
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-6xl w-full max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-gradient-to-r from-green-100 to-emerald-100 rounded-lg"><Users className="text-green-600" size={24} /></div>
            <div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="p-6 border-b border-gray-200">
            <div className="flex gap-3">
              ...
              <button onClick={handleStudentSearch} disabled={isSearching || !searchQuery.trim()}
                className="px-6 py-3 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl hover:shadow-lg transition font-medium disabled:opacity-50 flex items-center gap-2">
                {isSearching ? <><div className="animate-spin h-5 w-5 border-2 border-white border-t-transparent rounded-full" />Searching...</> : <><Search size={18} />Search</>}
              </button>
            </div>
            <div className="mt-4">
              ...
              <div className="flex flex-wrap gap-2">
                {exampleQueries.map((ex, idx) => (
                  <button key={idx} onClick={() => setSearchQuery(ex.query)}
          ...
          <div className="flex-1 overflow-y-auto p-6">
            {searchExplanation && (
              <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-xl">
                <div className="flex items-center gap-2 text-blue-700 mb-1"><Lightbulb size={16} /><span className="font-medium">AI Interpretation</span></div>
            ...
            {isSearching ? (
              ...
              <>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-semibold text-gray-900">Found {searchResults.length} student{searchResults.length !== 1 ? 's' : ''}</h3>
                  <span className="px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm">{searchResults.length} results</span>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {searchResults.map(student => (
                    <div key={student._id} className="border border-gray-200 rounded-xl p-4 hover:border-green-300 hover:shadow-md transition-all bg-white">
                      <div className="flex items-start gap-3 mb-3">
                        <div className="w-12 h-12 bg-gradient-to-r from-green-500 to-emerald-500 rounded-full flex items-center justify-center text-white font-bold flex-shrink-0 text-lg">
                          {student.name.charAt(0)}
                        </div>
                        <div className="flex-1">
                          <h4 className="font-bold text-gray-900">{student.name}</h4>
                          <p className="text-sm text-gray-600">Section: {student.section}</p>
                          {student.mentor?.name && <p className="text-xs text-indigo-600 font-medium mt-0.5">Mentor: {student.mentor.name}</p>}
                        </div>
                      </div>
                      <div className="space-y-2">
                        {(student as any).phone && (
                          <div className="flex items-center gap-2">
                            <svg xmlns="http://www.w3.org/2000/svg" className="text-gray-400" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.6 3.3 2 2 0 0 1 3.6 1h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L7.91 8.6a16 16 0 0 0 6 6l.91-.91a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
                            <span className="text-sm text-gray-700">{(student as any).phone}</span>
                          </div>
                        )}
                        {(student as any).email && (
                          <div className="flex items-center gap-2">
                            <Mail size={14} className="text-gray-400" />
                            <span className="text-sm text-gray-700 truncate">{(student as any).email}</span>
                          </div>
                        )}
                        {student.bloodGroup && (
                          <div className="flex items-center gap-2">
                        ...
                        {student.hobbies && student.hobbies.length > 0 && (
                          <div className="mt-2">
                            ...
                            <div className="flex flex-wrap gap-1">
                              {student.hobbies.slice(0, 3).map((h, i) => (
                                <span key={i} className="px-2 py-0.5 bg-gray-100 text-gray-700 rounded text-xs">{h}</span>
                              ))}
                              {student.hobbies.length > 3 && (
                                <span className="px-2 py-0.5 bg-gray-100 text-gray-700 rounded text-xs">+{student.hobbies.length - 3} more</span>
                              )}
                            </div>
                          </div>
                        )}
              ...
              <div className="text-center py-12">
                <div className="inline-flex items-center justify-center w-16 h-16 bg-gray-100 rounded-full mb-4"><Search className="text-gray-400" size={24} /></div>
                <h3 className="text-lg font-semibold text-gray-900 mb-2">No students found</h3>
                <p className="text-gray-600">Try a different query or check your spelling.</p>
              </div>
            ) : (
              <div className="text-center py-12">
                <div className="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-r from-green-100 to-emerald-100 rounded-full mb-4"><Users className="text-green-600" size={24} /></div>
                <h3 className="text-lg font-semibold text-gray-900 mb-2">Search for students</h3>
                <p className="text-gray-600 max-w-md mx-auto mb-6">Enter a natural language query to find students by location, section, batch, hobbies, or any attribute.</p>
                <div className="inline-grid grid-cols-2 gap-3">
                  {exampleQueries.slice(0, 4).map((ex, idx) => (
                    <button key={idx} onClick={() => setSearchQuery(ex.query)}
                      className="text-left p-4 bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded-xl transition">
                      <div className="flex items-center gap-2 mb-2">{ex.icon}<span className="text-sm font-medium text-gray-700">Example</span></div>
                      <p className="text-gray-900 font-medium">"{ex.query}"</p>
                    </button>
                  ))}
                </div>
              </div>
...
function formatBotResponse(text: string): string {
  return text
    ...
    .replace(/```(\w+)?\n([\s\S]*?)```/g, (_, lang, code) => {
      ...
      return `<div class="my-4 rounded-lg overflow-hidden border border-white/10"><div class="flex items-center justify-between px-4 py-2 bg-gray-900 border-b border-white/10"><span class="text-xs font-mono text-gray-400">${language}</span></div><pre class="bg-gray-900 p-4 overflow-x-auto"><code class="text-sm font-mono text-gray-200">${escapeHtml(code)}</code></pre></div>`;
    })
